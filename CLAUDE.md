...
**`eslint-disable`을 사용하지 마세요.**
어떻게 해야할지 모르겠으면 질문 기능을 사용해서 물어보세요.
...
lint와 싸우지 마세요.
이는 작업자가 코드를 보고 이해하고 찾기 쉽게 만들어줍니다.
파일 라인수 제한 (300 줄) 경고를 보면 파일을 분리하세요.
함수 라인수 제한 (50 줄) 경고를 보면 함수를 분리하세요.
...
--------------|---------|----------|---------|---------|-------------------
All files     |   99.23 |    90.32 |     100 |   99.23 |                   
 container.ts |   98.92 |       88 |     100 |   98.92 | 109               
 types.ts     |       0 |        0 |       0 |       0 |                   
--------------|---------|----------|---------|---------|-------------------
```
...
소스 파일과 마찬가지로 lint 검사 및 타입 유효성 검사가 통과해야 합니다.
그리고 `node <example.ts>`를 실행하고 프로세스의 정상 종료 및 콘솔 출력을 확인하는 테스트를 작성해주세요.
